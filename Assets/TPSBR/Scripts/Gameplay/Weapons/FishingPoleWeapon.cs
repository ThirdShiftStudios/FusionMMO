...
namespace TPSBR
{
    public class FishingPoleWeapon : Weapon
    {
        ...
        internal void LaunchLure()
        {
            ...
            // Ensure the lure can detect collisions with water surfaces even if the default weapon hit mask excludes them
            int waterLayer = ObjectLayer.Water;

            if (waterLayer >= 0)
            {
                hitMask |= 1 << waterLayer;
            }
        ...
        internal void OnLureImpacted(FishingLureProjectile projectile, in LagCompensatedHit hit)
        {
            ...
            if (_castActive == false && hitWater == false)
                return;
            ...
            if (projectile != null)
            {
                ...
                else if (projectile == _activeLureProjectile)
                {
