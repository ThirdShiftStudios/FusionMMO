    ...
    private bool TryAddWeaponDefinitionToInventory(WeaponDefinition definition)
    {
        ...
        private void EnsureFishingPoleInstance()
        {
            ...
            if (fishingPole != null)
            {
                ...
                if (_hotbar.Length > HOTBAR_FISHING_POLE_SLOT)
                {
                    bool fishingPoleShouldStayInHotbar =
                        _isFishingPoleEquipped == true || _currentWeaponSlot == HOTBAR_FISHING_POLE_SLOT;

                    if (fishingPoleShouldStayInHotbar == true && _hotbar[HOTBAR_FISHING_POLE_SLOT] != fishingPole)
                    {
                        AddWeapon(fishingPole, HOTBAR_FISHING_POLE_SLOT);
                    }
                }
