    ...
    public partial class TrelloTicketService
    {
        ...
        private bool IsConfigurationValid()
        {
            return string.IsNullOrWhiteSpace(Configuration.ApiKey) == false
                   ...
                   && string.IsNullOrWhiteSpace(Configuration.BaseUrl) == false;
        }
        ...
        {
            var apiBase = Configuration.BaseUrl?.TrimEnd('/') ?? "https://api.trello.com";
            var createCardUrl = new Uri(new Uri(apiBase + "/"), "1/cards");
            ...
            var request = UnityWebRequest.Post(createCardUrl, form);
        ...
        private async Task<TrelloSubmissionResult> SendRequestAsync(UnityWebRequest request)
        {
            ...
            var ticketId = ExtractTicketId(responseBody);
