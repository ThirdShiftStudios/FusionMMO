    ...
    partial class Tiff : IDisposable
    {
        ...
        public int ComputeTile(int x, int y, int z, short plane)
        {
            ...
            if (dx != 0 && dy != 0 && dz != 0)
            {
                ...
                else if (z != 0) //this is the original formula with x y x, it doesn't yield the correct index when z = 0
                    tile = (xpt * ypt) * (z / dz) + xpt * (y / dy) + x / dx;
                else //Gtiff files from Open Topography have z = 0, so simplify the formula to have the correct tile index
                    tile = xpt * y + x;
            }
